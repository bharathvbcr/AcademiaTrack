        ...
        <FieldSet legend="Faculty Contacts">
            <div className="md:col-span-2 space-y-2">
                {appData.facultyContacts.map((faculty, index) => {
                    const isExpanded = Boolean(isFacultyOpen[index]);
                    return (
                    <div key={faculty.id} className="bg-slate-100 dark:bg-slate-700/50 rounded-lg border border-slate-200 dark:border-slate-700">
                        <div className="flex items-center p-2">
                            <button type="button" onClick={() => setIsFacultyOpen(p => p.map((s, i) => i === index ? !s : s))} className="flex-grow flex items-center gap-2 text-left" aria-expanded={isExpanded ? "true" : "false"}>
                                        ...
                                        <div>
                                            ...
                                            <div className="flex items-center gap-4">
                                                <input
                                                    ...
                                                    aria-label="Research fit score from 1 to 10"
                                                />
                                            ...
                                            <div key={corr.id} className="bg-white dark:bg-slate-800 p-3 rounded border border-slate-200 dark:border-slate-600 text-sm">
                                                <div className="flex justify-between items-start mb-1">
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <button type="button" onClick={() => removeCorrespondence(index, corr.id)} className="text-slate-400 hover:text-red-500" aria-label={`Delete correspondence: ${corr.subject}`}>
