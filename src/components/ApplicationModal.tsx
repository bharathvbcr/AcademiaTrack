...
interface ApplicationModalProps {
  ...
  onDuplicate?: (app: Application) => void;
}

const ApplicationModal: React.FC<ApplicationModalProps> = ({ isOpen, onClose, applicationToEdit, onSave, onDuplicate }) => {
  const {
  ...
  // Track initial data to detect unsaved changes
  const initialDataRef = useRef<Application | null>(null);
  useEffect(() => {
    if (isOpen && applicationToEdit) {
      initialDataRef.current = JSON.parse(JSON.stringify(applicationToEdit));
    } else if (isOpen && !applicationToEdit) {
      initialDataRef.current = null;
    }
  }, [isOpen, applicationToEdit]);

  const hasUnsavedChanges = () => {
    if (!applicationToEdit && initialDataRef.current === null) {
      // New application - check if any fields are filled
      return appData.universityName.trim() !== '' || 
             appData.programName.trim() !== '' ||
             appData.deadline !== null;
    }
    if (!initialDataRef.current) return false;
    
    // Compare current appData with initial data
  ...
  const handleDuplicate = () => {
    if (onDuplicate && applicationToEdit) {
      onDuplicate(applicationToEdit);
      onClose();
    }
  };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" aria-labelledby="modal-title" role="dialog" aria-modal="true">
          ...
          >
            <div className="flex items-center justify-between p-5 border-b border-slate-200 dark:border-slate-700">
              ...
              <button type="button" onClick={onClose} aria-label="Close modal" className="p-1.5 rounded-full text-slate-500 hover:bg-slate-200 dark:hover:bg-slate-700 hover:text-slate-700 dark:hover:text-slate-200 transition-colors">
