...
export const useApplications = () => {
  ...
  // Track changed application IDs for incremental search index updates
  const changedAppIdsRef = React.useRef<Set<string>>(new Set());
  const deletedAppIdsRef = React.useRef<Set<string>>(new Set());
  ...
  // Get changed and deleted IDs for search index updates
  const getChangedAppIds = () => {
    const changed = Array.from(changedAppIdsRef.current);
    const deleted = Array.from(deletedAppIdsRef.current);
    changedAppIdsRef.current.clear();
    deletedAppIdsRef.current.clear();
  ...
  return {
    ...
    canUndo,
    canRedo,
    getChangedAppIds, // Expose for search index updates
  };
};
